...
func TestGracefulShutdownTimeoutValidation(t *testing.T) {
	driver := NewDriver("default", "path").(*Driver)

	// Test with valid timeout (0 or positive)
	checkFlags := &drivers.CheckDriverOptions{
		FlagsValues: map[string]interface{}{
			"vmwarevsphere-graceful-shutdown-timeout": 60,
			"vmwarevsphere-creation-type":             "legacy",
		},
		CreateFlags: driver.GetCreateFlags(),
	}
	err := driver.SetConfigFromFlags(checkFlags)
	assert.NoError(t, err)
	assert.Equal(t, 60, driver.GracefulShutdownTimeout)

	// Test with zero timeout (should be valid)
	checkFlags = &drivers.CheckDriverOptions{
		FlagsValues: map[string]interface{}{
			"vmwarevsphere-graceful-shutdown-timeout": 0,
			"vmwarevsphere-creation-type":             "legacy",
		},
		CreateFlags: driver.GetCreateFlags(),
	}
	err = driver.SetConfigFromFlags(checkFlags)
	assert.NoError(t, err)
	assert.Equal(t, 0, driver.GracefulShutdownTimeout)

	// Test with negative timeout (should fail)
	checkFlags = &drivers.CheckDriverOptions{
		FlagsValues: map[string]interface{}{
			"vmwarevsphere-graceful-shutdown-timeout": -1,
			"vmwarevsphere-creation-type":             "legacy",
		},
		CreateFlags: driver.GetCreateFlags(),
	}
	err = driver.SetConfigFromFlags(checkFlags)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "can not be negative")
}
